...
abstract class Email
{
	...
	public static function classToUse( string $type ): string
	{
		...
		{
			if( ( Settings::i()->mail_method == $key AND $class::isUsable( $type ) )
				OR ( $key == 'sendgrid' and $class::isUsable( $type ) ) // SendGrid is a special use case since it can send bulk mail only.
				OR ( $key == 'debug' and $class::isUsable( \IPS\Email::TYPE_TRANSACTIONAL ) ) )
			{
				return $class;
			}
		}
