...
trait Statistics
{
	...
	public function rebuildPostedIn( array $inSet, $members=array() )
	{
		...
		try
		{
			...
			/* If we don't have member(s) in the where clause, or we have a lot of member ids in the where clause, we can't do individual inserts as this could be 10,000+ inserts on very large topics */
			if ( ! count( $memberIds ) or ( count( $memberIds ) > 100 ) )
			{
			...
			{
				foreach( \IPS\Db::i()->select( "DISTINCT `{$authorColumn}` as {$authorColumn}, `{$commentItemField}`", $commentClass::$databaseTable, $where ) as $row )
				{
